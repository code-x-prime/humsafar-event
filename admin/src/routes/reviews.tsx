...
export function ReviewsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.get<Review[]>(`/admin/reviews?limit=100${statusFilter !== 'ALL' ? `&status=${statusFilter}` : ''}`),
  })

  const updateMutation = useMutation({
    mutationFn: ({ id, payload }: { id: string; payload: Record<string, unknown> }) => api.patch(`/admin/reviews/${id}`, payload),
    onSuccess: () => {
  ...
  return (
    <div>
      <h1 className="font-display text-2xl font-semibold text-primary">Reviews</h1>
      <p className="mt-1 text-sm text-muted-foreground">
        Customer reviews from completed orders — approve to show on the product page, reject to hide, or reply publicly.
      </p>
