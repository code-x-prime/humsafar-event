...
export function ProductsPage() {
  ...
  const toggleMutation = useMutation({
    mutationFn: ({ id, field, value }: { id: string; field: string; value: boolean }) =>
      api.patch(`/admin/products/${id}/toggle`, { field, value }),
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Full package listings shown on the client site — images, description, pricing, colors,
                ...
                <TableRow key={product.id}>
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-1">
                      <Button variant="ghost" size="sm" onClick={() => navigate(`/products/${product.id}`)} title="Edit">
