...
export interface TrendingShopProduct {
  id: string;
  title: string;
  slug: string;
  price: string;
...
export function TrendingShopProducts({ products }: { products: TrendingShopProduct[] }) {
    ...
    <section className="border-t border-(--ink-100) py-7 sm:py-10">
      <div className="mx-auto max-w-7xl px-4">
        <div className="flex items-end justify-between gap-4">
          <div>
            <h2 className="font-display text-xl font-semibold text-(--navy-800) sm:text-2xl">Trending Products</h2>
            <span className="mt-1.5 block h-0.75 w-10 rounded-full bg-(--coral-600)" />
          </div>
          <Link href="/shop" className="font-heading text-sm font-semibold text-accent hover:underline">
            View all
          </Link>
        </div>

        <Carousel opts={{ align: "start", dragFree: true }} className="mt-5">
          <CarouselContent>
            {products.map((product) => {
              ...
              const savings =
                discount !== null && product.mrp ? Math.round(Number(product.mrp) - Number(product.price)) : null;
                    ...
                    <div className="flex flex-col gap-2 p-3.5">
                      ...
                      <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                        ...
                        {savings !== null && savings > 0 && (
                          <span className="font-sans text-[11px] font-semibold text-(--success,#15803D)">
                            Save &#8377;{savings}
                          </span>
                        )}
                      </div>
