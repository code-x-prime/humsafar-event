...
interface RelatedResponse {
  similar: RelatedShopProduct[];
  youMayLike: RelatedShopProduct[];
}

function ProductRow({ title, products }: { title: string; products: RelatedShopProduct[] }) {
...
export function RelatedShopProducts({ slug }: { slug: string }) {
  const [data, setData] = useState<RelatedResponse | null>(null);

  useEffect(() => {
    getJson<RelatedResponse>(`/shop/products/${slug}/related`)
      .then(setData)
      .catch(() => {
  ...
  return (
    <>
      <ProductRow title="Similar Products" products={data.similar} />
