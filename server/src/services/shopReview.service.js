...
export async function update(id, data) {
  ...
  const updated = await prisma.shopProductReview.update({ where: { id }, data, include: INCLUDE });
...
export async function toggle(id, field, value) {
  ...
  const updated = await prisma.shopProductReview.update({ where: { id }, data: { [field]: value }, include: INCLUDE });
